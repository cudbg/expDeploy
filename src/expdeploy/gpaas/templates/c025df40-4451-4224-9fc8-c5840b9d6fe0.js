...
function checkResolution(minw, minh) {
  var w = $(window).width(), 
      h = $(window).height(),
      c = $("#container2"),
      o = c.offset(),
      cw = c.width(),
      ch = c.height();
  console.log([w, o.left+cw, h, o.top+ch]);
  if ((o.left + cw <= w) && (o.top + ch < h)) {
    return true;
  }
  console.log("crap, your browser is not big enough");
  $("#submitmsg").empty();
  return false;
}
  ...
  var render = function(ropt) {
    ...
    function _render() {
      ...
      $("#submit").click(function() {
        ...
        if (opt.results == null) {
          $("#submitmsg").text("Please ensure each bar has an estimate between 5 and 95");
          return;
        }
        $("#submit").off("click");
        onSubmit(opt);
